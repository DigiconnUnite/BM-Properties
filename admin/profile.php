<?php

require_once __DIR__ . '/_bootstrap.php';

require_csrf_post();

$adminId = (int) (admin_user()['id'] ?? 0);
$admin = get_admin_by_id($adminId);
$settings = get_site_settings();
if (!$admin) {
  admin_logout();
  header('Location: login.php');
  exit;
}

$message = '';
$error = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $action = clean_text((string) ($_POST['action'] ?? 'update_profile'));

  if ($action === 'update_profile') {
    $username = clean_text((string) ($_POST['username'] ?? ''));
    $fullName = clean_text((string) ($_POST['full_name'] ?? ''));
    $email = strtolower(clean_text((string) ($_POST['email'] ?? '')));
    $facebookUrl = clean_text((string) ($_POST['facebook_url'] ?? '#'));
    $instagramUrl = clean_text((string) ($_POST['instagram_url'] ?? '#'));
    $youtubeUrl = clean_text((string) ($_POST['youtube_url'] ?? '#'));

    if ($username === '' || strlen($username) < 4) {
      $error = 'Username must be at least 4 characters.';
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
      $error = 'Please enter a valid email address.';
    } else {
      update_admin_profile($adminId, $username, $fullName, $email);
      save_site_settings([
        'office_address' => (string) ($settings['office_address'] ?? ''),
        'phone' => (string) ($settings['phone'] ?? ''),
        'email' => (string) ($settings['email'] ?? ''),
        'open_time' => (string) ($settings['open_time'] ?? ''),
        'facebook_url' => $facebookUrl,
        'instagram_url' => $instagramUrl,
        'youtube_url' => $youtubeUrl,
        'page_title_bg' => (string) ($settings['page_title_bg'] ?? 'images/banner/banner2.webp'),
      ]);
      $admin = get_admin_by_id($adminId);
      $settings = get_site_settings();
      admin_login($admin);
      $message = 'Profile updated successfully.';
    }
  }

  if ($action === 'change_password') {
    $currentPassword = (string) ($_POST['current_password'] ?? '');
    $newPassword = (string) ($_POST['new_password'] ?? '');
    $confirmPassword = (string) ($_POST['confirm_password'] ?? '');

    if (!password_verify($currentPassword, (string) $admin['password_hash'])) {
      $error = 'Current password is incorrect.';
    } elseif (strlen($newPassword) < 8) {
      $error = 'New password must be at least 8 characters.';
    } elseif (!hash_equals($newPassword, $confirmPassword)) {
      $error = 'New password and confirm password do not match.';
    } else {
      update_admin_password($adminId, password_hash($newPassword, PASSWORD_DEFAULT));
      $message = 'Password updated successfully.';
    }
  }

  if ($action === 'update_contact_settings') {
    if ($error === '') {
      save_site_settings([
        'office_address' => clean_text((string) ($_POST['office_address'] ?? '')),
        'phone' => clean_text((string) ($_POST['phone'] ?? '')),
        'email' => clean_text((string) ($_POST['email'] ?? '')),
        'open_time' => clean_text((string) ($_POST['open_time'] ?? '')),
        'page_title_bg' => (string) ($settings['page_title_bg'] ?? 'images/banner/banner2.webp'),
      ]);
      $settings = get_site_settings();
      $message = 'Contact settings updated successfully.';
    }
  }
}

$pageTitle = 'My Profile';
$activePage = 'profile';
$adminDisplayName = trim((string) ($admin['full_name'] ?? ''));
if ($adminDisplayName === '') {
  $adminDisplayName = (string) ($admin['username'] ?? 'Admin');
}
$adminUsername = (string) ($admin['username'] ?? '');
$adminEmail = (string) ($admin['email'] ?? '');
$contactEmail = (string) ($settings['email'] ?? '');
$facebookUrl = (string) ($settings['facebook_url'] ?? '#');
$instagramUrl = (string) ($settings['instagram_url'] ?? '#');
$youtubeUrl = (string) ($settings['youtube_url'] ?? '#');

require_once __DIR__ . '/_layout.php';
admin_layout_top($pageTitle, $activePage);
?>
<div class="main-content profile-page-layout">
  <section class="admin-card profile-hero-card">
    <div class="profile-avatar" aria-hidden="true">
      <img src="../images/logo/logo-new2.webp" alt="BM Properties logo">
    </div>
    <div class="profile-hero-copy">
      <span class="profile-hero-kicker">Administrator Profile</span>
      <h2 class="profile-hero-name"><?php echo htmlspecialchars($adminDisplayName, ENT_QUOTES, 'UTF-8'); ?></h2>
      <p class="profile-hero-description">Manage your personal details, contact information, and password in one streamlined place.</p>
      <div class="profile-meta">
        <div class="profile-meta-item">
          <span>Role</span>
          <strong>Admin</strong>
        </div>
        <div class="profile-meta-item">
          <span>Username</span>
          <strong><?php echo htmlspecialchars($adminUsername, ENT_QUOTES, 'UTF-8'); ?></strong>
        </div>
        <div class="profile-meta-item">
          <span>Email</span>
          <strong><?php echo htmlspecialchars($adminEmail, ENT_QUOTES, 'UTF-8'); ?></strong>
        </div>
      </div>
    </div>
  </section>

  <?php if ($message !== ''): ?>
    <div class="alert alert-success profile-alert"><?php echo htmlspecialchars($message, ENT_QUOTES, 'UTF-8'); ?></div>
  <?php endif; ?>
  <?php if ($error !== ''): ?>
    <div class="alert alert-danger profile-alert"><?php echo htmlspecialchars($error, ENT_QUOTES, 'UTF-8'); ?></div>
  <?php endif; ?>

  <div class="profile-section-grid">
    <section class="admin-card profile-section-card">
      <div class="profile-section-head">
        <h2 class="profile-section-title">Personal Information</h2>
        <p>Update the core admin identity used across the panel.</p>
      </div>
      <form method="post" class="admin-form-grid profile-form-grid">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8'); ?>">
        <input type="hidden" name="action" value="update_profile">
        <div>
          <label for="full_name">Name</label>
          <input id="full_name" class="form-control" name="full_name"
            value="<?php echo htmlspecialchars((string) ($admin['full_name'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"
            maxlength="140">
        </div>
        <div>
          <label for="username">Username</label>
          <input id="username" class="form-control" name="username" required
            value="<?php echo htmlspecialchars((string) ($admin['username'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"
            maxlength="80">
        </div>
        <div class="admin-form-full">
          <label for="profile_email">Email</label>
          <input id="profile_email" type="email" class="form-control" name="email" required
            value="<?php echo htmlspecialchars((string) ($admin['email'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>" maxlength="120">
        </div>
        <div class="admin-form-full">
          <div class="profile-subsection-title">Social Links</div>
          <div class="profile-social-grid">
            <div>
              <label for="facebook_url">Facebook URL</label>
              <input id="facebook_url" class="form-control" name="facebook_url"
                value="<?php echo htmlspecialchars($facebookUrl, ENT_QUOTES, 'UTF-8'); ?>">
            </div>
            <div>
              <label for="instagram_url">Instagram URL</label>
              <input id="instagram_url" class="form-control" name="instagram_url"
                value="<?php echo htmlspecialchars($instagramUrl, ENT_QUOTES, 'UTF-8'); ?>">
            </div>
            <div>
              <label for="youtube_url">YouTube URL</label>
              <input id="youtube_url" class="form-control" name="youtube_url"
                value="<?php echo htmlspecialchars($youtubeUrl, ENT_QUOTES, 'UTF-8'); ?>">
            </div>
          </div>
        </div>
        <div class="admin-form-full profile-section-actions">
          <button class="btn btn-primary admin-btn profile-action-btn" type="submit">
            <i class="fa-solid fa-floppy-disk"></i>
            <span>Save Profile</span>
          </button>
        </div>
      </form>
    </section>

    <section class="admin-card profile-section-card">
      <div class="profile-section-head">
        <h2 class="profile-section-title">Contact Info</h2>
        <p>Keep the office contact details current.</p>
      </div>
      <form method="post" class="admin-form-grid profile-form-grid">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8'); ?>">
        <input type="hidden" name="action" value="update_contact_settings">
        <div class="admin-form-full">
          <label for="office_address">Office Address</label>
          <textarea id="office_address" class="form-control" name="office_address"
            rows="2"><?php echo htmlspecialchars((string) ($settings['office_address'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></textarea>
        </div>
        <div>
          <label for="phone">Phone</label>
          <input id="phone" class="form-control" name="phone"
            value="<?php echo htmlspecialchars((string) ($settings['phone'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
        </div>
        <div>
          <label for="contact_email">Email</label>
          <input id="contact_email" class="form-control" name="email"
            value="<?php echo htmlspecialchars($contactEmail, ENT_QUOTES, 'UTF-8'); ?>">
        </div>
        <div class="admin-form-full">
          <label for="open_time">Opening Time</label>
          <input id="open_time" class="form-control" name="open_time"
            value="<?php echo htmlspecialchars((string) ($settings['open_time'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
        </div>
        <div class="admin-form-full profile-section-actions">
          <button class="btn btn-primary admin-btn profile-action-btn" type="submit">
            <i class="fa-solid fa-floppy-disk"></i>
            <span>Save Contact Info</span>
          </button>
        </div>
      </form>
    </section>

    <section class="admin-card profile-section-card profile-section-password">
      <div class="profile-section-head">
        <h2 class="profile-section-title">Change Password</h2>
        <p>Update the admin password without affecting any other settings.</p>
      </div>
      <form method="post" class="admin-form-grid profile-form-grid profile-password-grid">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8'); ?>">
        <input type="hidden" name="action" value="change_password">
        <div>
          <label for="current_password">Current Password</label>
          <input id="current_password" class="form-control" type="password" name="current_password" required
            autocomplete="current-password">
        </div>
        <div>
          <label for="new_password">New Password</label>
          <input id="new_password" class="form-control" type="password" name="new_password" required minlength="8"
            autocomplete="new-password">
        </div>
        <div>
          <label for="confirm_password">Confirm New Password</label>
          <input id="confirm_password" class="form-control" type="password" name="confirm_password" required minlength="8"
            autocomplete="new-password">
        </div>
        <div class="admin-form-full profile-section-actions">
          <button class="btn btn-primary admin-btn profile-action-btn" type="submit">
            <i class="fa-solid fa-shield-halved"></i>
            <span>Update Password</span>
          </button>
        </div>
      </form>
    </section>
  </div>
</div>
<?php admin_layout_bottom(); ?>
